...
2. **[15 points]** Consider the PDE: $$x u_x + y u_y = 2u, x > 0, y > 0.$$
   ...
   a. $u = 1$ on the hyperbola $xy = 1$. 
   b. $u = 1$ on the circle $x^2 + y^2 = 1$. 
   c. Can you solve the equation, in general, if certain initial data is prescribed on the initial curve $y = e^x$? Justify with reasons.

3. **[20 points]** Sketch the characteristic curve, the initial curve, and solve the following problems: 
   a. $x u_x + y u_y = k u, \; x \in \mathbb{R}, \; y \geq \alpha > 0; \; u(x, \alpha) = F(x)$, where $k$, $\alpha$ are fixed and $F$ is a given smooth function. 
   b. $y u_x - x u_y = 0; \; u(x, 0) = x^2$. 
   c. $x^2 u_x - y^2 u_y = 0; \; u(1, y) = F(y)$. 
   d. $y u_x + x u_y = 0; \; u(0, y) = e^{-y^2}$. 

4. **[10 points]** Solve the quasi-linear problem and verify transversality conditions: 
   a. $u u_x + u_y = 0, \; u(x, 0) = x$. 
   b. $u u_x + u_y = 1, \; u(x, x) = x/2, \; x \in (0, 1]$. 